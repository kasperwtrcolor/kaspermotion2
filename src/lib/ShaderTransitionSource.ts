...
export const SHADER_LIBRARY: Record<string, string> = {
  ...
  "ridged-burn": H + NQ + `float ridged(vec2 p){float v=0.,a=.5;mat2 R=mat2(.8,.6,-.6,.8);
  ...
  "sdf-iris": H + `void main(){vec4 A=texture2D(u_from,v_uv),B=texture2D(u_to,v_uv);
  ...
  "ripple-waves": H + `void main(){vec2 uv=v_uv-.5;float dist=length(uv);vec2 dir=normalize(uv+.001);
  ...
  "gravitational-lens": H + `void main(){vec4 B=texture2D(u_to,v_uv);
  ...
  "cinematic-zoom": H + `void main(){vec2 d=v_uv-vec2(.5);
  ...
  "chromatic-split": H + `void main(){vec2 c=v_uv-.5;
  ...
  "glitch": H + `float rand(vec2 co){return fract(sin(dot(co,vec2(12.9898,78.233)))*43758.5453);}
  ...
  "swirl-vortex": H + NQ + `void main(){vec2 uv=v_uv-.5;float dist=length(uv);
  ...
  "thermal-distortion": H + NQ + `void main(){float heat=u_progress*1.5;
  ...
  "flash-through-white": H + `void main(){vec4 A=texture2D(u_from,v_uv),B=texture2D(u_to,v_uv);
  ...
  "cross-warp-morph": H + NQ + `void main(){vec2 disp=vec2(fbm(v_uv*3.),fbm(v_uv*3.+vec2(7.3,3.7)))-.5;
  ...
  "light-leak": H + `vec3 aces(vec3 x){return clamp((x*(2.51*x+.03))/(x*(2.43*x+.59)+.14),0.,1.);}
    void main(){vec4 A=texture2D(u_from,v_uv),B=texture2D(u_to,v_uv);
    vec2 lp=vec2(1.3,-.2);float dist=length(v_uv-lp);
    float leak=clamp(exp(-dist*1.8)*u_progress*4.,0.,1.);
    vec3 warmColor=mix(vec3(0.8, 0.4, 0.1), vec3(1.0, 0.8, 0.6), dist*.7);
    float flare=exp(-abs(v_uv.y-(-.2+v_uv.x*.3))*15.)*leak*.3;
    vec3 overexposed=A.rgb+warmColor*leak*3.+vec3(1.0, 0.9, 0.8)*flare;
